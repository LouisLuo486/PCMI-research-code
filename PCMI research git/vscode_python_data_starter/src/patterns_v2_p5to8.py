...
if not cc.empty and 'vin' in claims_min.columns:
    tmp = cc[[col_claim_id, col_cpsi_sel]].dropna().copy()
    tmp[col_claim_id] = as_str(tmp[col_claim_id]); tmp[col_cpsi_sel] = as_str(tmp[col_cpsi_sel])

    attach_cols = [col_claim_id,'vin']
    if 'policyholder_id' in claims_min.columns: attach_cols.append('policyholder_id')
    if col_servicer: attach_cols.append(col_servicer)

    tmp = tmp.merge(claims_min[attach_cols], on=col_claim_id, how='left').dropna(subset=['vin'])

    if col_servicer in tmp.columns:
        grp = tmp.groupby([col_servicer, col_cpsi_sel]).agg(
            distinct_vins=('vin','nunique'),
            distinct_policyholders=('policyholder_id','nunique') if 'policyholder_id' in tmp.columns else ('vin','size'),
            claims=('vin','size')
        ).reset_index().rename(columns={col_servicer:'servicer_id', col_cpsi_sel:'seller_id'})
...
p7_path = save_csv(p7_pairs, 'p7_cluster_pairs.csv')
...
if not p7_pairs.empty and not claims_min.empty and 'vin' in claims_min.columns and not cc.empty and col_servicer:
    # 构造 claim→(servicer,seller,vin) 明细
    tmp = cc[[col_claim_id, col_cpsi_sel]].dropna().copy()
    tmp[col_claim_id] = tmp[col_claim_id].astype(str)
    tmp[col_cpsi_sel] = tmp[col_cpsi_sel].astype(str)
    attach = claims_min[[col_claim_id, 'vin', col_servicer]].dropna().copy()
    attach[col_claim_id] = attach[col_claim_id].astype(str)
    attach['vin'] = attach['vin'].astype(str)
    tmp = tmp.merge(attach, on=col_claim_id, how='left').dropna(subset=['vin', col_servicer])
    tmp = tmp.rename(columns={col_cpsi_sel:'seller_id', col_servicer:'servicer_id'})
    ...
    pairs = p7_pairs.rename(columns={p7_pairs.columns[0]:'servicer_id', p7_pairs.columns[1]:'seller_id'})[key]
    tmp = tmp.merge(pairs, on=key, how='inner')

    # 统计每个 pair 在 P5/P6 VIN 上的覆盖
    p7_pairs_hits = (tmp.groupby(key)
                        .agg(distinct_vins_total=('vin','nunique'),
                             distinct_vins_in_p5p6=('in_p5_or_p6_vin','sum'),
                             claims=('vin','size'))
                        .reset_index())
    # 只保留真的“命中”P5/P6 的 pair
    p7_pairs_hits = p7_pairs_hits[p7_pairs_hits['distinct_vins_in_p5p6'] > 0] \
                        .sort_values(['distinct_vins_in_p5p6','distinct_vins_total','claims'], ascending=False)
...
if not p7_pair_claims_set:
    # 尝试通过 claim -> (servicer, seller) 与 p7_pairs 做匹配
    if 'p7_pairs' in globals() and not p7_pairs.empty and 'cc' in globals() and not cc.empty:
        # 统一列
        p7_pairs_std = p7_pairs.rename(columns={p7_pairs.columns[0]:'servicer_id',
                                               p7_pairs.columns[1]:'seller_id'})
        need_cols = [col_claim_id, col_servicer]
        claim_pairs = claims_min[need_cols].dropna().copy() if all(c in claims_min.columns for c in need_cols) else pd.DataFrame()
        if not claim_pairs.empty and 'icoverageid' in cc.columns or True:
            # 从 cc 里取 seller
            if col_claim_id in cc.columns:
                cc2 = cc[[col_claim_id, col_cpsi_sel]].dropna().copy()
                cc2[col_claim_id] = cc2[col_claim_id].astype(str)
                cc2[col_cpsi_sel] = cc2[col_cpsi_sel].astype(str)
                claim_pairs[col_claim_id] = claim_pairs[col_claim_id].astype(str)
                claim_pairs[col_servicer]  = claim_pairs[col_servicer].astype(str)
                claim_pairs = claim_pairs.merge(cc2.rename(columns={col_cpsi_sel:'seller_id'}),
                                                on=col_claim_id, how='left').dropna(subset=['seller_id'])
                hit = claim_pairs.merge(p7_pairs_std[['servicer_id','seller_id']],
                                        left_on=[col_servicer,'seller_id'],
                                        right_on=['servicer_id','seller_id'], how='inner')
                p7_pair_claims_set = set(hit[col_claim_id].astype(str))
    # 若没有 seller 映射，P7 无法落到 claim 级 —— 结果可能为空
...
if not claims_min.empty:
    keep = [col_claim_id]
    if 'vin' in claims_min.columns:        keep.append('vin')
    if col_servicer in claims_min.columns: keep.append(col_servicer)
    if col_claim_date in claims_min.columns: keep.append(col_claim_date)
    p3to8_df = p3to8_df.merge(claims_min[keep].drop_duplicates(subset=[col_claim_id]),
                              on=col_claim_id, how='left')
...
summary_rows = [
    ["P5_vin_bursts (events)",               _safe_len(p5_bursts)],
    ["P6_cross_shop (events)",               _safe_len(p6_cross)],
    ["P7_cluster_pairs (pairs)",             _safe_len(p7_pairs)],
    ["P8_invoice_hashes (hashes)",           _safe_len(p8_hash)],
    ["P8_invoice_flagged_claims (claims)",   _safe_len(p8_claims)],
    # ===== Intersections =====
    ["P5∩P6 VIN (vins)",                     _safe_len(vin_p5_p6)],
    ["P5∩P6 flagged claims (claims)",        _safe_len(p5p6_claims)],
    ["P7 pairs ∩ (P5|P6) (pairs)",           _safe_len(p7_pairs_hits)],
    ["P8 claims ∩ (P5|P6) (claims)",         _safe_len(p8_claims_hits)],
    ["P8 hashes ∩ (P5|P6) (hashes)",         _safe_len(p8_hash_hits)],
    ["P5∩P6∩P8 (claims)",                    _safe_len(p5p6p8_claims)],
    ["P3∩P4∩P5∩P6∩P7∩P8 (claims)", len(p3to8_df)],
    ]

summ_df = pd.DataFrame(summary_rows, columns=['metric','count'])
sum_path = OUT_DIR / 'summary_counts_v2.csv'
